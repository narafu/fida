...
- **OCI 상시 기동 서버로 이전 완료 (2026-08-04 커트오버 검증됨)**: `.github/workflows/server-deploy.yml`(main 브랜치 push 시 자동 배포, fida 자체 스케줄 실행 시간대(화~토 06:50~07:30 KST)에는 배포 차단, 헬스게이트+자동 롤백) + `deploy/server/{docker-compose.yml,Caddyfile,README.md}`. 배포 대상 `/opt/fida/`(kista-api와 **별도**의 독립 OCI 인스턴스 `168.110.113.178`, 자체 Caddy·자체 예약 공인 IP), `FIDA_DOMAIN=fida.kista-app.com`. 2026-08-04 07:00 KST 첫 네이티브 스케줄러 실행에서 배포 결함 2건 발견·수정 완료: ① secrets 파일 소유권이 non-root 컨테이너 UID/GID(appuser, 통상 100:101)와 안 맞아 `Permission denied` — README에 `chown` 절차 문서화, ② 서버 `.env` 개행 누락으로 `KISTA_URL`이 `INTERNAL_API_TOKEN`에 흡수돼 조용히 죽은 `kista-api.fly.dev`로 폴백 — `application.yml` 기본값을 `api.kista-app.com`으로 수정 + README에 `cat -A` 확인 경고 추가. 같은 날 수동 트리거(`POST /api/fida/orders`)로 스크래핑·OCR·시트·텔레그램·KISTA 저장까지 전체 파이프라인 성공 검증됨
...
- **healthchecks.io dead-man's switch 설정 완료 (2026-08-04)** — `HeartbeatPort`/`HeartbeatAdapter`가 `FandingScheduler.run()` 성공 직후 핑 전송(`HEARTBEAT_URL` 미설정 시 핑 생략, 실패는 로그만 남기고 삼킴). 체크 slug `fida-scheduler`, schedule `0 7 * * 2-6` (tz `Asia/Seoul`, grace 1시간) — 화~토 07:00 KST 기준. `HEARTBEAT_URL`을 로컬·서버(`/opt/fida/.env`) 양쪽에 반영 후 서버 컨테이너 재생성 완료, ping 자체 도달 확인됨(`status: up`). 실제 스케줄 실행을 통한 전체 검증은 다음 실행(2026-08-05 07:00 KST)에서 확인 필요
