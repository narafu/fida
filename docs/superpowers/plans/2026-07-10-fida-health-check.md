...
**오케스트레이션(메인 세션): Sonnet 5** — 태스크별 서브에이전트 파견, 태스크 사이 diff 리뷰, 실패 시 재파견 판단. 이 계획은 코드가 전부 명시돼 있어 오케스트레이터의 역할은 "리뷰 게이트"이며, Fable/Opus까지는 불필요하고 Haiku로는 리뷰 품질이 부족함.

| 태스크 | 실행 모델 | 근거 |
|--------|----------|------|
| Task 1 (scrape-server.js) | `haiku` | 전체 파일 교체 코드 제공됨. 검증 명령도 명시됨 |
| Task 2 (연도 경계) | `haiku` | 테스트·구현 코드 완전 제공, 단일 파일 쌍 |
| Task 3 (OCR 텔레그램 승격) | `haiku` | 3파일 수정이지만 모든 코드 블록 제공됨 |
| Task 4 (CI 워크플로) | `haiku` | 파일 1개 신규 생성, 내용 제공됨 |
| Task 5 (예외 핸들러) | `haiku` | 테스트·구현 코드 완전 제공 |
| Task 6 (문서 정합화) | `haiku` | 기계적 문자열 교체 4건 |
| Task 7 (KistaPort 일원화) | `sonnet` | 유일하게 판단 필요 — 기존 테스트 10여 개를 깨지 않게 수정·삭제하고 `processPost()` 블록 평탄화. 컴파일 오류 연쇄 대응 필요 |

**운영 규칙:** Haiku 태스크가 2회 연속 실패(테스트 불통과·BOM 삽입 등)하면 같은 태스크를 Sonnet으로 승격 재파견. 오케스트레이터는 각 태스크 커밋 전 diff를 직접 읽고 계획과 대조할 것.
