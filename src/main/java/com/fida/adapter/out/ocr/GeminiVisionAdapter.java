...
public class GeminiVisionAdapter implements OcrPort {
    ...
    private static final String PROMPT =
            "이 이미지는 주식 거래 기록입니다. 아래 JSON 형식으로만 응답하세요 (다른 텍스트 없이 JSON만):\n" +
                    ...
                    "- current_cycle_start: 이미지에서 정확히 \"현사이클 시작 $\" 라벨(달러기호 포함)인 행의 값만 사용. 근처에 \"XXXX 시작원금 $\"(연도+시작원금) 라벨의 행이 별도로 있으며 값이 다름 — 해당 행은 사용 금지. 이미지 좌측 상단의 \"현사이클시작\"(공백·달러기호 없음) 라벨 행은 사이클 시작 날짜(M/D 형식, 예: 7/10)이며 전혀 다른 항목 — 반드시 제외. 날짜가 아닌 금액임.\n" +
                    "- season_start_capital: 이미지에서 \"XXXX 시작원금 $\"(연도 또는 시즌N+시작원금) 라벨 행의 값. current_cycle_start와 혼동 검증용이므로 별도로 기록. 없으면 null\n" +
                    "- capital_rows: 오른쪽 상단 자금 표의 라벨/금액 행을 보이는 그대로 모두 기록. 예: [{\"label\":\"시즌1 시작원금\",\"value\":10000.00},{\"label\":\"현사이클 시작\",\"value\":11783.18},{\"label\":\"잔금\",\"value\":8283.77}]. 이미지 좌측 상단의 \"현사이클시작\"(공백 없음) 라벨 행은 \"현사이클시작 | N/M\"(분수) 또는 \"7/10\"(날짜) 등 금액이 아닌 형태로 나타나며, 이 표와 다른 위치의 별개 항목이므로 capital_rows에 절대 포함하지 말 것\n" +
                    ...
                    "- holding_qty: 이미지에 정확히 \"보유개수\" 또는 \"현재 보유 개수\" 라벨이 보일 때만 그 값을 기록. 라벨이 없으면 반드시 null. \"매수개수\"/\"직전 매매 개수\" 값을 복사하지 말 것\n" +
                    ...
                    "- holdings: 최종 보유 수량. \"누적개수\" 값을 우선 사용하고, 없을 때만 \"보유개수\" 또는 \"현재 보유 개수\" 값 사용. \"매수개수\"/\"직전 매매 개수\" 사용 금지(음수일 수 있어 혼동 주의). 반드시 0 이상의 정수";
